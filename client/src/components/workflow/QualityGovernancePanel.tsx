...
export function QualityGovernancePanel({ projectId, blindTag, currentPhaseKey, permissions, onRuntimeRefresh }: { projectId: string; blindTag: string; currentPhaseKey: string; permissions: any; onRuntimeRefresh: () => Promise<unknown> | void }) {
  ...
  return <Card className="overflow-hidden border-border">
    ...
    <CardContent className="space-y-5 p-4">
      ...
      <RecordSection title="Defect Notifications" icon={<FileWarning className="h-4 w-4 text-red-600" />} records={quality?.defects ?? []} numberKey="defectNumber" titleKey="title" statusKey="status" onAdd={canWork && permissions.canRecordDefect ? () => open("defect") : undefined} canEdit={canWork && (permissions.canRecordDefect || permissions.canReviewDefect)} onEdit={(record) => open("defect", record)} />
      <RecordSection title="Punch Items" icon={<ShieldCheck className="h-4 w-4 text-amber-600" />} records={quality?.punches ?? []} numberKey="punchNumber" titleKey="title" statusKey="status" onAdd={canWork && permissions.canManagePunch ? () => open("punch") : undefined} canEdit={canWork && (permissions.canManagePunch || permissions.canVerifyPunch)} onEdit={(record) => open("punch", record)} />
      <RecordSection title="NDT Records" icon={<Activity className="h-4 w-4 text-violet-600" />} records={quality?.ndt ?? []} numberKey="ndtNumber" titleKey="method" statusKey="status" onAdd={canWork && permissions.canRecordNdt ? () => open("ndt") : undefined} canEdit={canWork && (permissions.canRecordNdt || permissions.canReviewNdt)} onEdit={(record) => open("ndt", record)} />
    </CardContent>
    <QualityDialog kind={kind} record={editing} defects={quality?.defects ?? []} pending={defect.isPending || punch.isPending || ndt.isPending} onClose={close} onSave={(value) => {
